...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const checkConnection = async () => {
      if (window.ethereum && window.ethereum.selectedAddress) {
        try {
          ...
          // We'll set authenticated to false here until we verify the user type from contract
          // For now, just check if we have a wallet address
          if (connectedAddress) {
            // In a real app, we would check user type from contract
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      // In a production app, we would verify if user exists in contract
      const savedUserType = localStorage.getItem("e-certify-user-type");
      if (savedUserType && (savedUserType === "student" || savedUserType === "institute")) {
        setUserType(savedUserType);
        setIsAuthenticated(true);
